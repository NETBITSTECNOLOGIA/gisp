...
function Permissao($item, $id)
{
    ...
    if (mysqli_num_rows($sql) >= 1) {
        mysqli_query($conexao, "update permissao set valor='ativo' where usuario='" . $id . "' and item='" . $item . "' ");
    } else {
        mysqli_query($conexao, "insert into permissao (usuario,item,valor) VALUES ('$id','$item','ativo') ");
    }
};
