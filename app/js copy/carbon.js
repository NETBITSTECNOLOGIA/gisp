var _carbonads = {
    ...
    nonempty: function(v) { return this.isset(v) && v != "" },
    refresh: function() {
    ...
    remove: function(el) { if (typeof el !== 'undefined' && el != null) el.parentNode.removeChild(el) },
    htmlEncode: function(v) { return v.replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/'/g, '&apos;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\//g, '&#x2F;') }
};
...
function _carbonads_go(b) {
    ...
    image = _carbonads.isset(ad.logo) ? ad.logo : _carbonads.isset(ad.image) ? ad.image : _carbonads.isset(ad.smallImage) ? ad.smallImage : ad.fallbackImage;
    link = _carbonads.isset(ad.statlink) ? ad.statlink.split('?encredirect=') : ad.fallbackLink.replace(/^(http:|https:)/gm, '');
    description = _carbonads.isset(ad.description) ? ad.description : _carbonads.isset(ad.title) && ad.title.length > 30 ? ad.title : _carbonads.isset(ad.description) ? ad.description : ad.fallbackTitle;
