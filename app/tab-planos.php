  ...
  while ($dd = mysqli_fetch_array($query)) {
    echo '
        <tr>
            ...
            <td>' . Real($dd['valor']) . '</td>
            <td>';
    if ($_SESSION['tipouser'] == 'Admin' or PermissaoCheck($idempresa, 'planos-editar', $iduser) == 'checked') {
      echo ' 
                <a href="#" onclick="alterar(' . $dd['id'] . ')" class="btn btn-social-icon btn-edit" title="alterar"><i class="fa fa-pencil"></i></a>&emsp;';
    }
    if ($_SESSION['tipouser'] == 'Admin' or PermissaoCheck($idempresa, 'planos-excluir', $iduser) == 'checked') {
      echo '
