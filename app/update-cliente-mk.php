...
if ($_POST['login'] != '') {
  ...
  mysqli_query($conexao, "UPDATE user SET user='$login' WHERE id='$iduser'") or die(mysqli_error($conexao));
}
if ($_POST['senha'] != '') {
  $senha = md5($_POST['senha']);
  //se a senha vir preenchido
  mysqli_query($conexao, "UPDATE user SET senha='$senha' WHERE id='$iduser'") or die(mysqli_error($conexao));
}
...
if (!empty($id) || !empty($nome) || !empty($contato)) {
  ...
  mysqli_query($conexao, "UPDATE user SET nome='$nome',fantasia='$fantasia',email='$email',cpf_cnpj='$cpf_cnpj',isestadual='$isestadual',ismunicipal='$ismunicipal',
    ...
    cep='$cep',rua='$rua',bairro='$bairro',cidade='$cidade',estado='$estado' WHERE id='$iduser'")
