...
include_once('conectar-servidor.php');
...
while ($dd = mysqli_fetch_array($query)) {
    echo '
            <tr>
                ...
                <td>' . $dd['ip'] . '</td>
                <td>';
    if ($tipouser == 'Admin' || PermissaoCheck($idempresa, 'servidor-editar', $iduser) == 'checked') {
        echo '
                        <a href="#" onclick="alterarServidor(' . $dd['id'] . ')" class="btn btn-social-icon btn-edit"><i class="fa fa-edit"></i></a>&ensp;';
    }
    if ($tipouser == 'Admin' || PermissaoCheck($idempresa, 'servidor-excluir', $iduser) == 'checked') {
        echo '
    ...
    if (PermissaoCheck($idempresa, 'planos', $iduser) == 'checked' or $_SESSION['tipouser'] == 'Admin') {
        echo '
    ...
    if (PermissaoCheck($idempresa, 'clientes-off', $iduser) == 'checked' or $_SESSION['tipouser'] == 'Admin') {
        echo '                    
