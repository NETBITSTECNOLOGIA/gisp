...
############################################################################
  ...
  if(mysqli_num_rows($query2) == 1){//dados banco
      ...
      if($retPrivado['jurosapos'] != '0.00'){ $jurosapos = limpar($retPrivado['jurosapos']); }else{ $jurosapos = 0; }
      if($retPrivado['valordesconto'] != '0.00'){ $valordesconto = Intval(limpaCPF_CNPJ($retPrivado['valordesconto'])); }else{ $valordesconto = 0; }
    ...
    echo persona('Dados Gerencianet incompletos');
  }
