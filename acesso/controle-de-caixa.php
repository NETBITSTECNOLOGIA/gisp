...
if ($_SESSION['tipouser'] == 'Admin' || PermissaoCheck($idempresa, 'controle-caixa-cadastrar', $iduser) == 'checked') {
  echo '
...
if ($caixa < -1) {
  echo '<p style="color:red"><b>Total em caixa: R$ ' . Real($caixa) . '</b></p>';
} else {
  echo '<p style="color:green"><b>Total em caixa: R$ ' . Real($caixa) . '</b></p>';
}
echo '
        </div>
...
if ($caixadia < -1) {
  echo '<p style="color:red"><b>Total em caixa: R$ ' . Real($caixadia) . '</b></p>';
} else {
  echo '<p style="color:green"><b>Total em caixa: R$ ' . Real($caixadia) . '</b></p>';
}
echo '
    </div>
</div>
...
if (mysqli_num_rows($sql) >= 1) {
  while ($dd = mysqli_fetch_array($sql)) {
    echo '<tr>
                                ...
                                <td>';
    if ($dd['tipo'] == 'Entrada') {
      echo '<i class="text-green">Entrada</i>';
    } else {
      echo '<i class="text-red">Sáida</i>';
    }
    echo '</td>
                                <td>' . $dd['banco'] . '</td>
                                ...
                                <td>';
    if ($_SESSION['tipouser'] == 'Admin' || PermissaoCheck($idempresa, 'controle-caixa-excluir', $iduser) == 'checked') {
      echo '
                                <a href="#" onclick="excluir(' . $dd['id'] . ')" title="excluir"><i class="fa fa-trash fa-2x text-red"></i></a>&ensp;';
    }
    if ($_SESSION['tipouser'] == 'Admin' || PermissaoCheck($idempresa, 'controle-caixa-exibir', $iduser) == 'checked') {
      echo '
