...
if (PermissaoCheck($idempresa, 'clientes-alterar-chamado', $iduser) == 'checked' or $_SESSION['tipouser'] == 'Admin') {
  echo '
                    <a href="#" class="btn btn-primary" onclick="alterarChamado(' . $id . ')"><i class="fa fa-edit fa-2x"></i> Editar</a>';
}
echo '
                    </div>
                        <div class="box-body">
...
if ($retorno['img1'] != '' or $retorno['img2'] != '' or $retorno['pdf'] != '') {
  echo '
...
if (mysqli_num_rows($query) >= 1) {
  while ($ddl = mysqli_fetch_array($query)) {
    if (@$datacad != $ddl['datacad']) {
      echo '<li class="time-label"><span class="bg-blue">' . dataForm($ddl['datacad']) . '</span></li>';
    }
    echo '
            <li>
    ...
    if ($ddl['situacao'] == 'ABERTO') {
      echo '<a class="btn btn-info btn-flat">' . $ddl['situacao'] . '</a>';
    }
    if ($ddl['situacao'] == 'SOLUCIONADO') {
      echo '<a class="btn btn-success btn-flat">' . $ddl['situacao'] . '</a>';
    }
    if ($ddl['situacao'] == 'PENDENTE' or $ddl['situacao'] == 'PENDENTE TERCEIRO') {
      echo '<a class="btn btn-warning btn-flat">' . $ddl['situacao'] . '</a>';
    }
    if ($ddl['situacao'] == 'CANCELADO') {
      echo '<a class="btn btn-danger btn-flat">' . $ddl['situacao'] . '</a>';
    }
    if ($ddl['situacao'] == 'REABERTO') {
      echo '<a class="btn btn-default btn-flat">' . $ddl['situacao'] . '</a>';
    }
    echo '
                  </p>
                </div>
...
<script>
  ...
  function alterarChamado(id) {
    ...
    $.get('clientes-chamado-retorno-alterar.php', {
      id: id
    }, function(data) {
