...
funçoes não usadas
//sugestão de login
	$partes = explode(' ', $dd['nome']);
	$primeiroNome = array_shift($partes);
	$ultimoNome = array_pop($partes);
	$sugestaologin = $primeiroNome.'.'.$ultimoNome.$dd['id'];
*/
...
function todosClientes(){
    include_once('conexao.php');
    $query = mysqli_query($conexao,"SELECT contato FROM cliente WHERE idempresa='9999999999' AND contato <> ''") or die (mysqli_error($conexao));
...
function todosAtivo(){
    include_once('conexao.php');
    $query = mysqli_query($conexao,"SELECT contato FROM cliente WHERE idempresa='9999999999' AND situacao='Ativo' AND contato <> ''") or die (mysqli_error($conexao));
    $rows = mysqli_num_rows($query);
    if($rows >= 1){
        return $rows;
    }else{
...
function situacao($valor){
    if($valor == 'RECEBIDO'){ return '<small class="label label-success">'.$valor.'</small>';}
    elseif ($valor == 'PENDENTE') { return '<small class="label label-primary">'.$valor.'</small>'; }
    elseif ($valor == 'VENCIDO') { return '<small class="label label-danger">'.$valor.'</small>'; }
    elseif ($valor == 'CANCELADO') { return '<small class="label label-dark">'.$valor.'</small>'; }
}

function situacaoBanco($valor){
    if($valor == 1){ return $sitbanco = '1 - NORMAL'; }
    elseif($valor == 2){  return $sitbanco = '2 - MOVIMENTO CARTORIO'; }
    elseif($valor == 3){  return $sitbanco = '3 - EM CARTORIO'; }
    elseif($valor == 4){  return $sitbanco = '4 - TITULO COM OCORRENCIA DE CARTORIO'; }
    elseif($valor == 5){  return $sitbanco = '5 - PROTESTADO ELETRONICO'; }
    elseif($valor == 6){  return $sitbanco = '6 - LIQUIDADO'; }
    elseif($valor == 7){  return $sitbanco = '7 - BAIXADO'; }
    elseif($valor == 8){  return $sitbanco = '8 - TITULO COM PENDENCIA DE CARTORIO'; }
    elseif($valor == 9){  return $sitbanco = '9 - TITULO PROTESTADO MANUAL'; }
    elseif($valor == 10){  return $sitbanco = '10 - TITULO BAIXADO/PAGO EM CARTORIO'; }
    elseif($valor == 11){  return $sitbanco = '11 - TITULO LIQUIDADO/PROTESTADO'; }
    elseif($valor == 12){  return $sitbanco = '12 - TITULO LIQUID/PGCRTO'; }
    elseif($valor == 13){  return $sitbanco = '13 - TITULO PROTESTADO AGUARDANDO BAIXA'; }
    elseif($valor == 14){  return $sitbanco = '14 - TITULO EM LIQUIDACAO'; }
    elseif($valor == 15){  return $sitbanco = '15 - TITULO AGENDADO BB'; }
    elseif($valor == 16){  return $sitbanco = '16 - TITULO CREDITADO'; }
    elseif($valor == 17){  return $sitbanco = '17 - PAGO EM CHEQUE - AGUARD.LIQUIDACAO'; }
    elseif($valor == 18){  return $sitbanco = '18 - PAGO PARCIALMENTE'; }
    elseif($valor == 19){  return $sitbanco = '19 - PAGO PARCIALMENTE CREDITADO'; }
    elseif($valor == 21){  return $sitbanco = '21 - TITULO AGENDADO OUTROS BANCOS'; }
    elseif($valor == 80){  return $sitbanco = '80 - EM PROCESSAMENTO (ESTADO TRANSITÓRIO)'; }
}
