...
//id cliente
$id = $_GET['id'];
$query = mysqli_query($conexao, "SELECT * FROM contratos WHERE idcliente='$id'") or die(mysqli_error);
...
mysqli_query($conexao, "UPDATE contratos SET situacao='Bloqueado', atualizado=NOW() WHERE idcliente='$id'") or die(mysqli_error($conexao));
mysqli_query($conexao, "UPDATE cliente SET situacao='Bloqueado', atualizado=NOW() WHERE id='$id'") or die(mysqli_error($conexao));
echo persona('Cliente bloqueado');
