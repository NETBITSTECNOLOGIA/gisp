...
if (mysqli_num_rows($sql) >= 1) {
    while ($dd = mysqli_fetch_array($sql)) {
        ...
        if ($dd['datapagamento'] != '0000-00-00') {
            echo dataForm($dd['datapagamento']);
        }
        echo '</td>
                            <td>';
        if ($dd['situacao'] != 'RECEBIDO') {
            if (PermissaoCheck($idempresa, 'cobranca-receber', $_SESSION['iduser']) == 'checked' || $_SESSION['tipouser'] == 'Admin') {
                echo '
...
<script>
    ...
    function excluirCobrancaSemBoleto(id) {
        ...
        if (r == true) {
            $.get('cancelar-sem-boleto.php', {
                id: id
            }, function(data) {
                $('#retorno').show().fadeOut(6000).html(data);
                window.setTimeout(function() {
                    $('.reclickar').trigger('click');
                }, 2001);
            });
        }
        return false;
