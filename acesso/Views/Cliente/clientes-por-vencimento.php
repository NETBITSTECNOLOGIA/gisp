...
<div class="content-wrapper">  
    ...
    <section class="content">  
      <div class="row">
      <form method="post" action="clientes-por-vencimento.php">  
      
          <div class="col-lg-2 col-md-2 col-sm-12 vencimento">Dia de vencimento
            <select type="text" class="form-control" name="vencimento">';
if (@$_POST['vencimento'] != '') {
  echo '<option value="' . @$_POST['vencimento'] . '">' . @$_POST['vencimento'] . '</option>';
} else {
  echo '<option value="">selecione</option>';
}
$sqlV = mysqli_query($conexao, "SELECT vencimento FROM cliente WHERE idempresa='$idempresa' GROUP BY vencimento ASC");
while ($r = mysqli_fetch_array($sqlV)) {
  echo '<option value="' . $r['vencimento'] . '">' . $r['vencimento'] . '</option>';
}
echo '
            </select>
          </div>
...
echo '
          ...
          <div class="col-lg-2 col-md-2 col-sm-12">Situação
...
$query = mysqli_query($conexao, "SELECT cliente.*, contratos.login AS logincontrato, user.nome AS nomeparceiro FROM cliente 
                    ...
                    WHERE cliente.idempresa='$idempresa' AND cliente.vencimento='$vencimento' AND cliente.situacao='$situacao' AND cliente.parceiro LIKE '%$parceiro' ORDER BY cliente.nome ASC")
...
if (mysqli_num_rows($query) >= 1) {
  ...
  echo '
                        <tr class="text-red"><td colspan="5">Sem registro</td></tr>
...
<script>
  ...
  $('#relatorios-por-vencimento').addClass('active');
