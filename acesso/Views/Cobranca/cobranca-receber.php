...
if (!empty($id) and !empty($valorpago)) {
    ...
    if ($_POST['banco'] == 'Gerencianet') {
        include_once_once('api_gerencianet.php');
        receberCobranca($id);
    }

    if ($_POST['banco'] == 'Banco Juno') {
        include_once_once('api_juno.php');
        receberCobranca($id, $valorpago, $datapagamento);
    }

    if ($_POST['banco'] == 'Banco do Brasil') {
        include_once_once('api_bb.php');
        receberCobranca($id, $valorpago, $datapagamento, $nomeuser);
    }

    if ($_POST['banco'] == 'Carteira') {
        //tabela cobranca
        mysqli_query($conexao, "UPDATE cobranca SET valorpago='$valorpago',datapagamento='$datapagamento',situacao='RECEBIDO' WHERE id='$id'") or die(mysqli_error($conexao));
