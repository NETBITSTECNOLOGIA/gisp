...
include_once_once('routeros_api.class.php');
...
while ($dd = mysqli_fetch_array($query)) {
    echo '
            <tr>
                ...
                <td>';
    if ($dd['dataatualizacao'] != '0000-00-00') {
        echo dataForm($dd['dataatualizacao']);
    }
    echo '</td>
                <td>';
    if ($_SESSION['tipouser'] == 'Admin' || PermissaoCheck($idempresa, 'historico-alterar', $iduser) == 'checked') {
        echo '
                    <a href="#" onclick="alterarHistorico(' . $dd['id'] . ')"><i class="fa fa-edit fa-2x"></i></a>&ensp;';
    }

    if ($_SESSION['tipouser'] == 'Admin' || PermissaoCheck($idempresa, 'historico-excluir', $iduser) == 'checked') {
        echo '
